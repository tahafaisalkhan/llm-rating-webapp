 ...
 *  - disabledInitial: kept but ignored (resubmissions allowed)
 */
export default function RubricForm({
  disabledInitial = false, // ignored now
  itemId,
  datasetId,
  comparison,
  modelUsed,
  ...
  const AXES = useMemo(
    () => [
      { label: "Medical Accuracy & Completeness",
        title: "All clinically relevant facts present, correct, not hallucinated (symptoms/history/findings/treatment)." },
      { label: "Clinical Safety & Handover Utility",
        title: "Safe to hand over (red-flags preserved; exact meds/labs/vitals; no dangerous omissions)." },
      { label: "Guideline Alignment & Clinical Reasoning",
        title: "Diagnosis/management align with guidelines; reasoning consistent with medical logic." },
      ...
      { label: "Structure, Flow & Communication",
        title: "Clear sectioning (S/O/A/P), chronology, speaker turns, explanations, key patient statements, respectful tone." },
      { label: "Communication, Rapport & Patient Engagement",
        title: "Clarity of explanations, respectful tone, empathy, participation, concerns addressed, education included." },
      { label: "Alignment to Source (“traceability”)",
        title: "Each note sentence traceable to dialogue; unsupported = hallucination/added knowledge." },
    ],
    []
  );
        ...
        <div className="space-y-2">
          {AXES.map((ax, i) => (
            <div key={ax.label} className="flex items-center justify-between gap-2" title={ax.title}>
              ...
              <Likert value={scores[i]} onChange={(v) => setAxis(i, v)} />
            </div>
          ))}
        </div>
      </div>
